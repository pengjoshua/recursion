...
var parseJSON = function(json) {
  ...
  var objects = function() {
    ...
    if (ch === '{') { // objects start with '{'
      ...
      while (ch) { // keys are strings
        key = strings(); // run strings function to process strings
        whitespace();
        next(':');
        if (Object.hasOwnProperty.call(object, key)) {
          throw new SyntaxError('key duplicate: "' + key + '"');
        }
        object[key] = datatype(); // add key to object, run datatype
        whitespace();
  ...
  var arrays = function() {
    ...
    if (ch === '[') { // arrays start with '['
      ...
      while (ch) {
        array.push(datatype()); // add element to array, run datatype
        whitespace();
  ...
  var numbers = function() {
    ...
    // exponent
    if (ch === 'E' || ch === 'e') { // check exponent E or e and add to string
      str += ch;
      next();
